...
build-backend: ## 构建后端（自动复制前端产物）
	...
	cd backend && $(GO) build -o ../bin/gateway-kiro .
...
release: build-web ## 编译 Linux 版本（用于上传到 Docker 部署）
	rm -rf backend/internal/gateway/webdist
	cp -r web/dist backend/internal/gateway/webdist
	cd backend && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(GO) build -buildvcs=false -trimpath -o ../bin/gateway-kiro-linux-amd64 .
	@echo "构建完成: bin/gateway-kiro-linux-amd64"
